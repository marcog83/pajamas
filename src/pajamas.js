(function (factory) {
	...
	var win = window, doc = document, rurl = /^([\w\+\.\-]+:)(?:\/\/([^\/?#:]*)(?::(\d+)|)|)/, readyState = 'readyState', xmlHttpRequest = 'XMLHttpRequest', xhr = win[xmlHttpRequest] ?
			function () { return new win[xmlHttpRequest]() } :
			function () { return new win.ActiveXObject('Microsoft.XMLHTTP') }, corsSupported = ('withCredentials' in xhr()), contentType = 'Content-Type', requestedWith = 'X-Requested-With', defaultHeaders = {
			contentType: 'application/x-www-form-urlencoded; charset=UTF-8', Accept: {
				'*': 'text/javascript, text/html, application/xml,' +
				' text/xml, */*', xml: 'application/xml, text/xml', html: 'text/html', text: 'text/plain', json: 'application/json, text/javascript', script: 'text/javascript, application/javascript,' +
			...
			var prop
        for (prop in d) {
            if (!hasOwn(o, prop) && hasOwn(d, prop)) {
                o[prop] = d[prop]
            }
        }
			return o
        ...
        if (typeof dataString !== 'string') {
			...
			var headers = options.headers || {}, accept = 'Accept', h
			...
			else if (!headers[requestedWith]) {
                delete headers[requestedWith]
            }
        ...
        if (verboseResolution) {
            return {
                response: response, status: http.status, xhr: http
            }
        } else {
            return response
        }
			...
			var parts = rurl.exec(url), defaultParts = rurl.exec(defaultUrl)
			return !!(parts &&
			(parts[1] !== defaultParts[1] || parts[2] !== defaultParts[2] ||
			(parts[3] || (parts[1] === 'http:' ? 80 : 443)) !==
			(defaultParts[3] || (defaultParts[1] === 'http:' ? 80 : 443 ))))
		}, sendLocal = function (o, resolve,reject) {
			var http = isFunction(o.xhr) ? o.xhr() : xhr(), timeoutVal, send = function () {
					try {
						http.send(o.data)
					} catch (err) {
						err.xhr = http
					...
					if (status >= 200 && status < 300 ||
                        ...
                        if (http.responseText) {
                            try {
                                ...
                                resolve(makeResolution(
                                    http, parser ? parser.call(http) : http, o.verboseResolution))
                            } catch (e) {
                                e.xhr = http
                                reject(e)
                            }
			...
			isNumeric(o.delay) ?
				...
				send()
		}, sendRemote = function (o, resolve,reject) {
			var head = document.head || document.getElementsByTagName('head')[0] ||
					document.documentElement, script = document.createElement('script'), callbackName, callback, timeoutVal, send = function () {
					script && head.appendChild(script)
				}
			script.async = 'async'
			if (o.dataType === 'jsonp') {
				callbackName = o.callbackName || 'pajamas' + (Math.floor(Math.random() * 1000)) +
			...
			script.onload = script.onreadystatechange = function (_, isAbort) {
				if (isAbort || !script.readyState ||
					...
					if (!isAbort) {
						...
						reject(new Error(o.url + ' aborted'))
					}
				}
			}
			script.onerror = function () {
				reject(new Error('Error loading ' + o.url))
			}
			if (isNumeric(o.timeout)) {
				timeoutVal = setTimeout(function () {
					script.onload(0, 1)
			...
			var  o = options == null ? {} : defaults({}, options), defaultUrl = (function () {
					var anchor
					try {
			...
			var promise = new pajamas.Promise(function (resolve, reject) {
                ...
                if (o.dataType === 'jsonp' ||
                    ...
                    (o.crossDomain && !corsSupported)) {
                    sendRemote(o, resolve, reject)
                } else {
                    sendLocal(o, resolve, reject)
                }
			})
			return promise
				.then(function (value) {
	...
	pajamas.param = function (data) {
		var prefix, queryStringBuilder = [], push = function (key, value) {
	...
	pajamas.val = function (el) {
		...
		if (el.nodeName.toLowerCase() === 'select') {
			v = (function () {
				var v, vals = [], selectedIndex = el.selectedIndex, opt, options = el.options, isSingleSelect = el.type === 'select-one', i, max
                if (selectedIndex < 0) {
                    return null
                }
	...
	pajamas.serializeArray = function () {
		var arr = [], i, el, crlf = /\r?\n/g, checkableType = /radio|checkbox/i, pushAll = function (elems) {
				var el, v, i, j
						...
						if (v != null) {
							if (isArray(v)) { // from multiple select, for instance
								for (j = 0; j < v.length; j++) {
									arr.push({
										name: el.name, value: v[j].replace(crlf, '\r\n')
									})
								}
							}
							else {
								arr.push({
									name: el.name, value: v.replace(crlf, '\r\n')
								})
							}
						}
					}
